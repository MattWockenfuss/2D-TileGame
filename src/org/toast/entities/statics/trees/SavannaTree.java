...
public class SavannaTree extends StaticEntity{
	
	private static int scale = 2; 
	
	public SavannaTree(Handler handler, float x, float y) {
		super(handler, x, y, Assets.trees[3].getWidth() * scale, Assets.trees[3].getHeight() * scale);
		//lets scale the tree
		bounds.x = 43 * scale;
		bounds.y = 65 * scale;
		bounds.width = 8 * scale;
		bounds.height = 16 * scale;
		isSolid = false;
		//hasGravity = false;
	...
	public void render(Graphics g) {
		g.drawImage(Assets.trees[3], (int) (x - handler.getGameCamera().getxOffset()),(int) (y - handler.getGameCamera().getyOffset()),width,height,null);
