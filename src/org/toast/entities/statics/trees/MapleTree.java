...
public class MapleTree extends StaticEntity{
	
	private static int scale = 2; 
	
	public MapleTree(Handler handler, float x, float y) {
		super(handler, x, y, Assets.trees[2].getWidth() * scale, Assets.trees[2].getHeight() * scale);
		//lets scale the tree
		bounds.x = 26 * scale;
		bounds.y = 47 * scale;
		bounds.width = 12 * scale;
		bounds.height = 34 * scale;
		isSolid = false;
		//hasGravity = false;
	...
	public void render(Graphics g) {
		g.drawImage(Assets.trees[2], (int) (x - handler.getGameCamera().getxOffset()),(int) (y - handler.getGameCamera().getyOffset()),width,height,null);
