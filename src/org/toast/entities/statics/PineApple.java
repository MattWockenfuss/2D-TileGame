...
public class PineApple extends StaticEntity{
	
	private int stage = 0;
	private int life = 0;
	
	public PineApple(Handler handler, float x, float y) {
		super(handler, x, y,Assets.pineappleCrop[0].getWidth(), Assets.pineappleCrop[0].getHeight());
	...
	public void render(Graphics g) {
		g.drawImage(Assets.pineappleCrop[stage], (int) (x - handler.getGameCamera().getxOffset()),(int) (y - handler.getGameCamera().getyOffset()),width,height,null);
	}
	...
	public void spawnChildren() {
		...
		if(stage == 2) {//Only if fully grown
			...
			handler.getWorld().getEntityManager().getEntities().add(
					new ItemEntity(handler,(float)centerX, (float)centerY, 55 ,2));
		}
